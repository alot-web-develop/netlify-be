...
function validateEmailRequest(requestBody) {
  ...
  if (!name || typeof name !== 'string' || name.trim().length === 0) {
    throw new EmailValidationError('Name is required and must be a non-empty string', 'name');
  }
  ...
  if (!message || typeof message !== 'string' || message.trim().length === 0) {
    throw new EmailValidationError('Message is required and must be a non-empty string', 'message');
  }
  ...
  for (const [index, fileLink] of fileLinks.entries()) {
    console.log(`Validating fileLink at index ${index}:`, JSON.stringify(fileLink, null, 2));
    ...
    const hasFileFormat = fileLink.fileName && (fileLink.downloadUrl || fileLink.viewUrl);
    
    console.log(`Has legacy format (filename, link): ${hasLegacyFormat}`);
    console.log(`Has image format (fileName, imgSrc): ${hasImageFormat}`);
    console.log(`Has file format (fileName, downloadUrl/viewUrl): ${hasFileFormat}`);
    
    if (!hasLegacyFormat && !hasImageFormat && !hasFileFormat) {
      console.error(`Invalid fileLink structure at index ${index}:`, fileLink);
      throw new EmailValidationError(
        `File link at index ${index} must have either (filename, link), (fileName, imgSrc), or (fileName, downloadUrl/viewUrl) properties. Received: ${JSON.stringify(fileLink)}`,
        'fileLinks'
      );
    }
  }
...
function formatEmailContent(formData) {
  ...
  if (fileLinks.length > 0) {
    ...
    fileLinks.forEach((fileLink) => {
      ...
      const link = fileLink.link || fileLink.imgSrc || fileLink.downloadUrl || fileLink.viewUrl;
      text += `- ${filename}: ${link}\n`;
    });
  }
...
function createEmailTransporter() {
  ...
  if (!GMAIL_HOST || !GMAIL_USER || !GMAIL_PASS) {
    throw new EmailSendError('Email configuration is incomplete. Check GMAIL_HOST, GMAIL_USER, and GMAIL_PASS environment variables.');
  }
 ...
 */
async function sendNotificationEmail(formData, recipientEmail = 'olamidedentaltechnology@gmail.com') {
  try {
...
async function saveConsentToSheets(consentData, spreadsheetId) {
  ...
  try {
    ...
    throw new GoogleAPIError(`Failed to save consent to sheets: ${error.message}`);
  }
}
 ...
 */
function createEmailErrorResponse(statusCode, message, corsHeaders, details = null) {
  const body = {
    status: 'error',
    message
  };
