...
function validateUploadRequest(requestBody) {
  ...
  if (!fileSize || typeof fileSize !== 'number' || fileSize <= 0) {
    throw new ValidationError('fileSize is required and must be a positive number', 'fileSize');
  }

  if (!mimeType || typeof mimeType !== 'string') {
    throw new ValidationError('mimeType is required and must be a string', 'mimeType');
  }
 ...
 * @param {string} params.targetFolderId - Google Drive folder ID
 ...
 */
async function createGoogleDriveSession({ fileName, fileSize, mimeType, targetFolderId, accessToken }) {
  const fileMetadata = {
    name: fileName,
  ...
  try {
    ...
    if (response.statusCode !== 200) {
      ...
      throw new GoogleDriveError(`${errorMessage} (${response.statusCode})`, response.statusCode);
    }
    ...
    throw new GoogleDriveError(`Failed to create Google Drive session: ${error.message}`);
  }
}
 ...
 */
function createUploadResponse({ uploadUrl, sessionId, fileName, fileSize, mimeType, corsHeaders }) {
  return {
    statusCode: 200,
    headers: corsHeaders,
    body: JSON.stringify({
