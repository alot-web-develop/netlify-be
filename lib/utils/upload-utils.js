...
function validateUploadRequest(requestBody) {
  ...
  // Check file size limit (6MB for Netlify Functions)
  const MAX_FILE_SIZE = 6 * 1024 * 1024; // 6MB in bytes
  if (fileSize > MAX_FILE_SIZE) {
    throw new ValidationError(
      `File size exceeds maximum limit of ${Math.round(MAX_FILE_SIZE / 1024 / 1024)}MB. Current size: ${Math.round(fileSize / 1024 / 1024)}MB`,
      "fileSize"
    );
  }
