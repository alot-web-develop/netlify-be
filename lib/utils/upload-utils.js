...
function validateUploadRequest(requestBody) {
  ...
  const MAX_SINGLE_UPLOAD_SIZE = CHUNK_SIZE;
  ...
  if (fileSize > MAX_TOTAL_FILE_SIZE) {
    throw new ValidationError(
      ...
      )}MB. Current size: ${Math.round(fileSize / 1024 / 1024)}MB`,
      "fileSize"
    );
  }
...
function generateNetlifyUploadUrl(sessionId, origin) {
  ...
  const baseUrl = "https://elegant-kulfi-ee6bb8.netlify.app";
  ...
  uploadSessions.set(sessionId, {
    ...
    chunks: sessionData.requiresChunking
      ? {
          ...
          totalChunks: Math.ceil(sessionData.fileSize / CHUNK_SIZE),
          uploadedChunks: 0,
          uploadedBytes: 0,
        }
      : null,
  ...
  if (requiresChunking) {
    ...
    response.chunking = {
      ...
      instructions: {
        ...
        example:
          "POST /.netlify/functions/upload-chunk with chunk data and range headers",
      },
    };
  } else {
