 ...
 */
async function isFilePublic(fileId) {
  try {
    const permissionsData = await googleAPIClient.getDriveFilePermissions(fileId);
...
async function makeFilePublic(fileId) {
  ...
  return googleAPIClient.createDriveFilePermission(fileId, permission);
}
 ...
 */
async function ensurePublicPermissions(fileId) {
  try {
    const isPublic = await isFilePublic(fileId);
...
function createImageObject(file) {
  ...
  return {
    imgSrc: `https://drive.google.com/thumbnail?id=${id}&sz=w1000`,
...
async function fetchImageFiles(folderId, options = {}) {
  const { ensurePublic = true, includeMetadata = false } = options;

  try {
    // Fetch files from the specified folder
    const response = await googleAPIClient.listDriveFiles(folderId, {
    ...
    const processedImages = await Promise.all(
      imageFiles.map(async (file) => {
        try {
          ...
          if (ensurePublic) {
            await ensurePublicPermissions(file.id);
          }
