 ...
 */
function getCorsHeaders(origin, allowedMethods, allowedHeaders = 'Content-Type, Authorization') {
  const methods = Array.isArray(allowedMethods) ? allowedMethods.join(', ') : allowedMethods;
...
function handleMethodNotAllowed(corsHeaders, allowedMethod) {
  return {
    statusCode: 405,
    headers: corsHeaders,
    body: JSON.stringify({ error: `Method Not Allowed - Use ${allowedMethod}` }),
  };
}
